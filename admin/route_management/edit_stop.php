...
<div class="card" style="max-width: 500px; margin: 0 auto;">
    <h2 style="color:var(--primary-blue);">Edit Shuttle Stop</h2>
    ...
    <form method="POST">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-top:15px;">
            <div><label>Latitude</label><input type="number" step="any" name="lat" class="form-control"
                    value="<?= $stop['lat'] ?>" required></div>
            <div><label>Longitude</label><input type="number" step="any" name="lng" class="form-control"
                    value="<?= $stop['lng'] ?>" required></div>
        </div>

        <label style="font-weight:600; margin-top:20px; display:block;">Linked Zones</label>
        <div
            style="border:1px solid #ccc; padding:10px; border-radius:4px; max-height:150px; overflow-y:auto; background:#fafafa;">
