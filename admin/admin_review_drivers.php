...
<head>
    ...
    <title>Review Driver Applications - Admin - CampusPulse</title>
        ...
        <div id="content">
            ...
            <div class="main-content">
                ...
                <?php if (isset($errorMsg)): ?>
                    <div class="alert alert-error" style="background: #f8d7da; color: #721c24; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #f5c6cb;">
                        <i class="fas fa-exclamation-triangle"></i> <?= htmlspecialchars($errorMsg) ?>
                    </div>
                <?php endif; ?>

                <?php if (isset($_SESSION['success'])): ?>
                    <div class="alert alert-success" style="background: #d4edda; color: #155724; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #c3e6cb;">
                ...
                <?php if (isset($_SESSION['error'])): ?>
                    <div class="alert alert-error" style="background: #f8d7da; color: #721c24; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #f5c6cb;">
                        <i class="fas fa-exclamation-triangle"></i> <?= htmlspecialchars($_SESSION['error']); unset($_SESSION['error']); ?>
                    </div>
                <?php endif; ?>
                        ...
                        <?php foreach ($pendingApplications as $app): ?>
                            <div class="driver-card">
                                ...
                                <div class="app-detail">
                                    ...
                                    <strong>Applied:</strong> <?= isset($app['applied_at']) ? htmlspecialchars(date('M j, Y, g:i a', strtotime($app['applied_at']))) : 'Unknown' ?>
                                </div>

                                <div class="docs-section">
                                    <h4><i class="fas fa-folder-open"></i> Uploaded Documents</h4>
                                    <div class="docs-links-grid">
                                        <a href="<?= htmlspecialchars(getSignedUrl($bucket, $app['doc_profile_pic'] ?? null)) ?>" target="_blank" class="doc-link">
                                            <i class="fas fa-image"></i> Profile Pic
                                        </a>
                                        <a href="<?= htmlspecialchars(getSignedUrl($bucket, $app['doc_ic'] ?? null)) ?>" target="_blank" class="doc-link">
                                            <i class="fas fa-address-card"></i> IC Copy
                                        </a>
                                        <a href="<?= htmlspecialchars(getSignedUrl($bucket, $app['doc_license'] ?? null)) ?>" target="_blank" class="doc-link">
                                            <i class="fas fa-id-badge"></i> License
                                        </a>
                                        <a href="<?= htmlspecialchars(getSignedUrl($bucket, $app['doc_psv'] ?? null)) ?>" target="_blank" class="doc-link">
                                ...
                                <div class="action-buttons">
                                    <a href="admin_view_application.php?id=<?= urlencode($app['id']) ?>" style="background: #003366; color: white; border: none; padding: 10px 15px; border-radius: 6px; text-align: center; text-decoration: none; width: 100%; display: block; font-weight: 600; transition: background 0.2s;">
