...
if (!$snapshot->exists()) die("Error: Admin profile not found.");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error) && isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $allowed)) {
            $cloudPath = 'staff_profiles/' . $adminId . '_' . time() . '.' . $ext;
            ...
            try {
                ...
                $updateData[] = ['path' => 'photo_url', 'value' => $publicUrl];
...
<head>
    <meta charset="UTF-8">
    <title>My Profile - CampusPulse</title>
    ...
    <div id="content">
        ...
        <div class="main-content">
            ...
            <div class="card" style="max-width: 850px; margin: 0 auto;">
                ...
                <?php if ($error): ?>
                    <div class="alert-error" style="background:#f8d7da; color:#721c24; padding:15px; border-radius:6px; margin-bottom:20px; border:1px solid #f5c6cb;">
                        <i class="fas fa-exclamation-triangle"></i> <?= htmlspecialchars($error) ?>
                    </div>
                <?php endif; ?>

                <?php if ($success): ?>
                    <div style="background:#d4edda; color:#155724; padding:12px; border-radius:6px; margin-bottom:20px; text-align:center; border:1px solid #c3e6cb;">
                ...
                <form method="POST" enctype="multipart/form-data" style="display:flex; gap:40px; flex-wrap:wrap;">
                    ...
                    <div style="flex:1; text-align:center; min-width:250px; border-right:1px solid #f0f0f0; padding-right:20px;">
                        <div style="position:relative; display:inline-block; margin-bottom:15px;">
                            <img src="<?= htmlspecialchars($admin['photo_url'] ?? '../assets/img/default-avatar.png') ?>" 
                                 id="previewImg"
                            ...
                            <label for="photoInput" style="position:absolute; bottom:10px; right:10px; background:var(--primary-blue); color:white; width:45px; height:45px; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; box-shadow:0 2px 5px rgba(0,0,0,0.2); transition:0.3s;">
                                <i class="fas fa-camera"></i>
                            </label>
                            <input type="file" name="photo" id="photoInput" style="display:none;" accept="image/*" onchange="previewFile(this)">
                        </div>
                        <h4 style="margin:0; color:var(--primary-blue);"><?= htmlspecialchars($admin['full_name']) ?></h4>
                    ...
                    <div style="flex:2; min-width:300px;">
                        ...
                        <div style="margin-bottom:15px;">
                            ...
                            <div style="position:relative;">
                                <input type="email" class="form-control" value="<?= htmlspecialchars($admin['email']) ?>" disabled style="background:#f4f4f4; color:#666; padding-left:40px;">
       ...
       style="position:absolute; right:15px; top:50%; transform:translateY(-50%); cursor:pointer; color:#999; z-index:10;">
                            ...
                            <small style="color:#666; display:block; margin-top:5px; font-size:0.85rem;">
                                <i class="fas fa-info-circle"></i> Must be at least 8 characters, include a capital letter (A-Z) and a symbol (!@#$).
                            </small>
                        </div>
