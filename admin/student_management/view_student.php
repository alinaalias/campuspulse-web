...
<div class="card">
            ...
            <div style="display:flex; justify-content:space-between; align-items:start;">
                ...
                <div class="status-badge">
                    <?php if (($student['status'] ?? 'active') === 'active'): ?>
                        <span class="badge"
                            style="background:var(--success); color:white; padding:6px 12px; border-radius:20px;">Active</span>
                    <?php else: ?>
                        <span class="badge"
                            style="background:var(--danger); color:white; padding:6px 12px; border-radius:20px;">Inactive</span>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    ...
    <div class="info-grid">
        <div class="info-group">
            ...
            <p><a href="mailto:<?= htmlspecialchars($student['student_email']) ?>"
    ...
    <div style="margin-top: 40px; border-top:1px solid #eee; padding-top:20px;">
        ...
        <?php if (!empty($student['status_update_reason'])): ?>
            <div
                style="background:#f9f9f9; border:1px solid #eee; border-left:4px solid var(--primary-blue); padding:15px; border-radius:8px; margin-bottom:20px;">
                <strong style="color:#555; font-size:0.85rem; display:block; margin-bottom:5px;">STATUS LOG</strong>
                <p style="margin:0; font-size: 0.95rem; color:#333;">
                    Reason: <strong><?= htmlspecialchars($student['status_update_reason']) ?></strong>
                    <br>
                    <span style="font-size:0.8rem; color:#888;">
                        (Updated by <?= htmlspecialchars($student['status_update_admin'] ?? 'System') ?> on
                        <?= formatDate($student['status_update_at'] ?? null) ?>)
                    </span>
                </p>
            </div>
        <?php endif; ?>
        ...
        <form method="POST" class="status-form">
            <div>
                ...
                <div style="font-size:0.85rem; color:#856404; margin-top:2px;">
                    To change status with a justification log, please use the toggle switch in the Students Management
                    table. Fast updates here will NOT record a reason.
                </div>
            </div>
